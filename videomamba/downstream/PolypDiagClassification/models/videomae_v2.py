...
MODEL_PATH = '/mnt/tqy/checkpoints/videomaev2'
_MODELS = {
    "videomaev2_s": os.path.join(MODEL_PATH, "vit_s_k710_dl_from_giant.pth"),
}
    ...
    def forward_features(self, x):
        ...
        if self.pos_embed is not None:
            x = x + self.pos_embed.expand(B, -1, -1).type_as(x).to(
                x.device).clone().detach()[:, :x.size(1)]
        x = self.pos_drop(x)
...
def load_state_dict(model, checkpoint, center=True):
    ...
    for k in checkpoint_model.keys():
        if k in state_dict_3d.keys() and checkpoint_model[k].shape != state_dict_3d[k].shape:
            if len(state_dict_3d[k].shape) <= 3:
                print(f'Ignore: {k}')
                continue
            print(f'Inflate: {k}, {checkpoint_model[k].shape} => {state_dict_3d[k].shape}')
            time_dim = state_dict_3d[k].shape[2]
            checkpoint_model[k] = inflate_weight(checkpoint_model[k], time_dim, center=center)
...
def vit_small_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load(_MODELS["videomaev2_s"], map_location='cpu')
