...
class PhaseDataset_AutoLaparo(Dataset):
    ...
    def __init__(
        ...
        self.dataset_samples = self._make_dataset(infos)
        del infos
    ...
    def _aug_frame(
        ...
        # print("aug_transform:", time.time() - t1)
        ...
        t1 = time.time()
        buffer = [F.to_tensor(img) for img in buffer]
        # print("totensor:", time.time() - t1)

        t1 = time.time()
        buffer = torch.stack(buffer)  # T C H W
        buffer = buffer.permute(0, 2, 3, 1)  # T H W C
        # print("shaoe:", time.time() - t1)
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        for i, _ in enumerate(range(0, self.clip_len)):
            ...
            if self.frame_sample_rate == -1:
                if self.mode == "train":
                    frame_sample_rate = random.randint(1, 3)
                else:
                    frame_sample_rate = 1
...
if __name__ == '__main__':
    ...
    dataset = PhaseDataset_AutoLaparo(
        anno_path="/mnt/tqy/AutoLaparo/AutoLaparo_Task1",
        data_path="/mnt/tqy/AutoLaparo/AutoLaparo_Task1",
        mode="train",  # Switch to 'val' or 'test' to test other modes
        data_strategy="online",  # Switch to 'offline' for offline testing
        output_mode="key_frame",  # Switch to 'all_frame' if needed
        clip_len=16,
        frame_sample_rate=2,
        args=args
    )
