...
class PhaseDataset_AutoLaparo_Sequence(Dataset):
    ...
    def __init__(
        ...
        infos = pickle.load(open(self.anno_path, "rb"))
        self.dataset_samples = self._make_dataset(infos, video_id)
        del infos
        if mode == "test":
    ...
    def __getitem__(self, index):
        if self.mode == "test":
            for frames_info in self.dataset_samples:
    ...
    def _make_dataset(self, infos, video_id):
        frames = []
        if video_id not in infos.keys():
            raise("Unknown video id {}".format(video_id))
        data = infos[video_id]
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                phase_label = int(self.dataset_samples[image_index]["phase_gt"])
                # PIL可视化
                # image_data.show()
                # cv2可视化
        ...
        return sampled_image_list, phase_data, sampled_list
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                if cut_black:
                    path = path.replace('frames', 'frames_cutmargin')
                image_data = Image.open(path)
                image_data = self.data_transform(image_data)
...
def build_dataset(is_train, test_mode, fps, args):
    if args.data_set == "AutoLaparo":
        ...
        dataset = PhaseDataset_AutoLaparo_Sequence(
            anno_path=anno_path,
...
if __name__ == '__main__':
    ...
    class Args:
        def __init__(self):
            self.aa = False  # AutoAugment flag (just a dummy value for testing)
            self.reprob = 0.25  # Random Erasing probability
            self.remode = 'pixel'  # Mode for Random Erasing
            self.recount = 1  # Count for Random Erasing
            self.data_path = "/mnt/tqy/AutoLaparo/AutoLaparo_Task1"
            self.data_set = "AutoLaparo"
            self.input_size = 224
    ...
    # Initialize PhaseDataset_AutoLaparo with dummy values for testing
    dataset = build_dataset(True, False, "1fps", args)

    # Create DataLoader to load a batch
    data_loader = DataLoader(dataset, batch_size=2, shuffle=True)

    # Testing __getitem__ by iterating over DataLoader
    for i, (buffer, label, identifier, flag) in enumerate(data_loader):
        print(f"Sample {i+1}:")
        print("Buffer shape:", buffer.shape)  # (B, T, H, W, C) or (T, C, H, W)
        print("Label:", label)
        print("Identifier:", identifier)
        print("Flag:", flag)
        print("\n")
        if i >= 1:  # Only fetch two batches for demonstration
            break