...
class LinearClassifierHead(nn.Module):
    def __init__(self, dim, num_classes, embed_dim=512, dropout_prob=0.5):
        super().__init__()
...
class EndoMamba(nn.Module):
    def __init__(
        ...
        if self.with_head:
            ...
            self.vis_head = LinearClassifierHead(self.num_features, num_classes * 2)
            self.pos_head = LinearRegressionHead(self.num_features, num_classes * 2)
        ...
        if self.with_head:
            self.vis_head.apply(segm_init_weights)
            self.pos_head.apply(segm_init_weights)
    ...
    def get_lr_params(self, base_lr, factor=1):
        ...
        if self.with_head:
            params.append({"params": self.vis_head.parameters(), "lr": base_lr})
            params.append({"params": self.pos_head.parameters(), "lr": base_lr})
        return params
    ...
    def forward(self, x, inference_params: Optional[List[Optional[Tensor]]] = None):
        ...
        if self.with_head:
            ...
            vis_pred = self.vis_head(x)
            vis_pos = self.pos_head(x)
            
            vis_pred = vis_pred.reshape([vis_pred.shape[0], vis_pred.shape[1], self.num_classes, 2])
            vis_pos = vis_pos.reshape([vis_pos.shape[0], vis_pos.shape[1], self.num_classes, 2])

            # Apply sigmoid to the visibility prediction (since it's a binary classification task)
            vis_pred = self.sigmoid(vis_pred)

        if inference_params is not None:
            return vis_pred, vis_pos, inference_params
        else:
            return vis_pred, vis_pos
