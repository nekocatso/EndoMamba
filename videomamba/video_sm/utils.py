...
class WandbLogger(object):
    def __init__(self, project_name, log_dir=None):
        # Initialize wandb with the given project name and optional log directory
        self.step = 0
    ...
    def update(self, head='scalar', step=None, **kwargs):
        # Prepare the data for logging, ensure that None values are skipped
        log_data = {}
        for k, v in kwargs.items():
            if v is None:
                continue
            if isinstance(v, torch.Tensor):
                v = v.item()  # Convert tensor to scalar if needed
            # if isinstance(v, dict):
            #     for kk, vv in v.items():
            #         log_data[head + "/" + kk] = vv
            #     break
            assert isinstance(v, (float, int))  # Ensure that values are numeric
            log_data[head + "/" + k] = v
        
        # Log the data to wandb
        wandb.log(log_data, step=self.step if step is None else step)
    
    def update_image(self, head, outputs, labels):
        # Convert to image-friendly format if needed and log the images
        wandb.log({
            f"{head}/outputs": [wandb.Image(outputs)],
            f"{head}/labels": [wandb.Image(labels)]
        }, step=self.step)
