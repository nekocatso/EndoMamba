...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if not ctx.has_z:
            ...
            return (out, x) if not return_last_state else (out, last_state)
        else:
            ctx.save_for_backward(u, delta, A, B, C, D, z, delta_bias, x, out, prev_state)
            out_z = rest[0]
            return (out, x, out_z) if not return_last_state else (out_z, last_state)
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if D is not None:
            D = D.contiguous()
        out, scan_intermediates, out_z = selective_scan_fn(
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                ...
                A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
                C_proj_bias=None, delta_softplus=True, ssm_state=None, conv_state=None,
                return_last_state=False, checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
             return_last_state=True need ssm_state and conv_state initial value and works only for inference
        """
        assert checkpoint_lvl in [0, 1]
        L = xz.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        ...
        if return_last_state:
            conv1d_out, conv_state = causal_conv1d_update_ref(x, conv_state, conv1d_weight, conv1d_bias, True)
        else:
            conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, True)
        # conv1d_out, conv_state = causal_conv1d_update_ref(x, conv_state, conv1d_weight, conv1d_bias, True)
        # We're being very careful here about the layout, to avoid extra transposes.
        ...
        if D is not None:
            D = D.contiguous()
        if return_last_state:
            out_z, *rest = selective_scan_fn(conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus, return_last_state, ssm_state)
            last_state = rest[0]
            out = F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
            return out if not return_last_state else (out, last_state.detach(), conv_state.detach())
        else:
            out, scan_intermediates, out_z = selective_scan_fn(
                conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus
            )
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if D is not None:
            D = D.contiguous()
        out_f, scan_intermediates_f, out_z_f = selective_scan_fn(
            conv1d_out, delta, A, B, C, D, z, delta_bias, delta_softplus
        )
        assert not A_b.is_complex(), "A should not be complex!!"
        out_b, scan_intermediates_b, out_z_b = selective_scan_fn(
...
def mamba_inner_fn(
    ...
    , ssm_state=None, conv_state=None, return_last_state=False
):
    return MambaInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus,
                              ssm_state, conv_state, return_last_state)
...
def mamba_inner_fn_no_out_proj(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True, ssm_state=None, conv_state=None, return_last_state=False
):
